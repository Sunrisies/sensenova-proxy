      ...
      {!loading && data && (
        <>
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-6">
            ...
            ].map(([label, value, color]) => <Card key={String(label)}><CardContent className="p-4"><div className="text-xs text-muted-foreground">{label}</div><div className={`mt-2 text-xl font-semibold ${color}`}>{formatNumber(Number(value))}</div></CardContent></Card>)}
          </div>
            ...
            <CardContent className="p-0">
              <div className="overflow-x-auto">
                <table className="w-full min-w-[760px] text-sm">
                  ...
                  <tbody>{data.stats.by_model.map((item) => <tr key={item.model} className="border-b last:border-0"><td className="px-5 py-4 font-medium">{item.model}</td><td className="px-3 py-4">{formatNumber(item.requests)}</td><td className="px-3 py-4 text-emerald-600">{formatNumber(item.successful_requests)}</td><td className="px-3 py-4 text-rose-600">{formatNumber(item.failed_requests)}</td><td className="px-3 py-4">{item.requests ? `${Math.round(item.successful_requests / item.requests * 1000) / 10}%` : "-"}</td><td className="px-3 py-4">{formatNumber(item.prompt_tokens)}</td><td className="px-3 py-4">{formatNumber(item.completion_tokens)}</td><td className="px-5 py-4 font-medium">{formatNumber(item.total_tokens)}</td></tr>)}</tbody>
                </table>
